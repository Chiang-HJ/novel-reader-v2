async function run() {
    ...
    // Check chapters 50-110 concurrently
    const promises = uniqueLinks.slice(50, 110).map(async (link, idx) => {
        const url = 'https://www.xbanxia.cc' + link;
        try {
            const r = await fetch(url);
            const html = await r.text();
            if (html.includes('木玄草') || html.includes('实不相瞒')) {
                console.log('Found in index', idx + 50, 'url:', url);
                ...
                if(match2) console.log(match2[0].replace(/\n/g, ' '));
            }
        } catch(e) {}
    });
